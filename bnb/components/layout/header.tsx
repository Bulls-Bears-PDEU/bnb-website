          ...
          <div className="hidden md:flex items-center space-x-4">
            {navItems.map((item, index) => (
              <Link key={item.name} href={item.href}>
                ...
                >
                  {/*bhai font size change kri ch pela aa hti font-medium capitalize>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> */}
                  <span className="text-xl lg:text-2xl font-semibold capitalize tracking-wid px-7 px-4">{item.name}</span>
                  {isActive(item.href) && (
                    <motion.div
